...
WEB_SCRAPER_DIR = ROOT_DIR / "webscraper"
if str(WEB_SCRAPER_DIR) not in sys.path:
    sys.path.append(str(WEB_SCRAPER_DIR))

# Load environment variables
load_dotenv()

# Import webscraper agent with safe fallback
try:
    from webscraper.agents import run_research
    webscraper_available = True
except Exception as e:
    print(f"⚠️ Webscraper agent import failed: {e}")
    run_research = None
...
def proxy_ai():
    ...
    try:
        ...
        if intent.get("mode") == "chat":
            response_data, status_code = handle_chat_mode(provider, prompt, {"gemini": gemini_api_key}, conversation_history)
...
# Dedicated endpoint to run the DeepResearcher webscraper agent without MCP routing
@app.route("/webscraper/research", methods=["POST", "OPTIONS"])
def webscraper_research():
    if request.method == "OPTIONS":
        return jsonify({"status": "ok"})

    try:
        if not webscraper_available:
            return jsonify({"error": "Webscraper agent not available on server (import failed)"}), 500
        ...
        if not os.getenv("SGAI_API_KEY"):
            missing_keys.append("SGAI_API_KEY")
        if missing_keys:
            return jsonify({"error": f"Missing required environment variables: {', '.join(missing_keys)}"}), 500
        ...
        # Execute the dedicated webscraper agent
        # Increase timeout for webscraper agent (longer-running - 10 minutes for complex scraping)
        response_text = run_research(prompt, timeout_seconds=600)

        # Surface common downstream errors and empty responses
        if isinstance(response_text, str) and "Insufficient credits" in response_text:
            return jsonify({
                "error": "Webscraper failed: Insufficient ScrapeGraph credits. Add credits to the SGAI_API_KEY account.",
                "conversation_id": conversation_id
            }), 200

        if isinstance(response_text, str) and response_text.startswith("Webscraper error:"):
            return jsonify({
                "error": response_text.replace("Webscraper error:", "").strip(),
                "conversation_id": conversation_id
            }), 200

        if not response_text:
            return jsonify({
                "error": "Webscraper returned no content.",
                "conversation_id": conversation_id
            }), 200
        ...
        print("Webscraper agent error:", err)
        traceback.print_exc()
        return jsonify({"error": str(err)}), 500
...
def handle_chat_mode(provider, prompt, api_keys, conversation_history=None):
    """
    Handle chat mode requests - returns a data dictionary, not a Flask response.
    Returns: (data_dict, status_code) tuple where status_code defaults to 200
    """
    try:
        # Check if provider is groq - use Modal endpoint instead of Groq API
        if provider == "groq":
            ...
            try:
                ...
                print(f"Modal endpoint call failed: {modal_error}")
                return {"error": f"Failed to call Modal endpoint: {str(modal_error)}"}, 500
        ...
        if not gemini_api_key:
            return {"error": "Gemini API key not configured"}, 400
