...
class DeepResearcherAgent(Workflow):
    ...
    def run(self, topic: str) -> Iterator:
        """
...
def _run_agent_once(query: str) -> str:
    ...
    for chunk in final_report_iterator:
        # Some versions return an object with .content, others may be plain strings
        content = getattr(chunk, "content", None)
        if content:
            full_report += content
        else:
            full_report += str(chunk)
    return full_report


def run_research(query: str, timeout_seconds: int = 600) -> str:
    """
    Run the deep researcher agent with a hard timeout and clear error surfacing.
    """
    try:
        future = _EXECUTOR.submit(_run_agent_once, query)
        return future.result(timeout=timeout_seconds)
    except FuturesTimeoutError:
        return "Webscraper error: timed out while fetching data. Please try again later."
